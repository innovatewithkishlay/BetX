...
export default function AdminLoginPage() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('betx_admin_remember_email');
        if (saved) setForm((p) => ({ ...p, email: saved, rememberMe: true }));
    }, []);

    useEffect(() => {
        // Redirection logic: if already logged in and verified as admin
        if (!authLoading && user) {
            // We'll trust the AuthContext for now, but in a real app 
            // we'd check the role property from the token/session
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            // Critical: Verify using the ADMIN endpoint
            const res = await api.post('/api/admin/auth/verify');

            if (res.data.success) {
                if (form.rememberMe) localStorage.setItem('betx_admin_remember_email', form.email);
                else localStorage.removeItem('betx_admin_remember_email');
            ...
            if (err.message?.includes('API key')) {
                ...
                setErrors({ general: 'Access denied. You do not have administrator rights.' });
                await auth.signOut();
            } else {
            ...
            <div style={boxStyle}>
                <div style={{ marginBottom: '32px' }}>
                    <div style={{
                        ...
                        <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--accent)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Administrator Terminal</span>
                    </div>
                    <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.025em' }}>BetX Admin Panel</h1>
                    <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '6px', fontWeight: 500 }}>Global system administration</p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', userSelect: 'none' }}>
                            ...
                            />
                            <span style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: 500 }}>Maintain persistent session</span>
                        </label>
                    </div>
                    ...
                    >
                        {loading ? 'Authorizing...' : 'Sign In as Administrator'}
                    </button>
                </form>
                ...
                <p style={{ textAlign: 'center', fontSize: '11px', color: 'var(--text-faint)', marginTop: '32px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Root Management Terminal v1.0.0
                </p>
            </div>
        </div>
    );
